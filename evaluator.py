...
def main():
    ...
    from config import IMAP_EMAIL, SENDER_EMAIL, RECIPIENT_EMAILS
    ...
    while True:
        try:
            check_count += 1
            print(f"\n🔄 Check #{check_count} — {datetime.now(PKT).strftime('%H:%M:%S')} PKT")
            ...
            print(f"  ⚠️  Error: {e} — retrying in {CHECK_INTERVAL}s...")
            time.sleep(CHECK_INTERVAL)
...
if __name__ == "__main__":
    while True:
        try:
            ...
            print(f"💥 Fatal: {e} — restarting in 30s...")
            time.sleep(30)
